...
// Endpoint temporal para probar el webhook manualmente
export async function GET(req: NextRequest) {
  console.log('🧪 TEST WEBHOOK - Llamada GET para probar')
  ...
  if (!raffleId || !numbers || !buyerId) {
    return NextResponse.json({ 
      error: 'Faltan parámetros: raffleId, numbers, buyerId' 
    }, { status: 400 })
  }
  
  const numbersArray = numbers.split(',').map(Number)
  
  console.log('🧪 TEST WEBHOOK - Datos recibidos:')
  ...
  try {
    ...
    const ticketRecords = numbersArray.map((number: number) => ({
      raffle_id: raffle.id,
      number: number,
      is_sold: true,
      buyer_id: buyerId
    ...
    console.log('✅ Tickets de prueba creados exitosamente:', numbersArray.join(', '))
    return NextResponse.json({ 
      success: true, 
      message: `Tickets creados: ${numbersArray.join(', ')}` 
    })
